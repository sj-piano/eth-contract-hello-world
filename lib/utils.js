...
function validateNumericString({ name, value }) {
  if (typeof value !== 'string') {
    throw new Error(`Received invalid value type for ${name}: value=${value}, type=${typeof value}`);
  }
  value = value.trim();
  if (value.length === 0) {
  ...
  if (!isNumericString(value)) {
    ...
    msg += `, !isNaN(value)=${!isNaN(value)}, !isNaN(parseFloat(value))=${!isNaN(parseFloat(value))}`;
    throw new Error(msg);
  }
  return value;
}
